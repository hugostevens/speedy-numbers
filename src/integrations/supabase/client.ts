      ...
      storage: {
        getItem: (key) => {
          try {
            ...
            // Basic validation to ensure it's a valid session object
            if (!item || (typeof item === 'object' && !item.access_token && !item.refresh_token)) {
              console.error('Invalid session data found in storage, removing it');
